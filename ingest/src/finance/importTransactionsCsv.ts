...
const parseAmountNumber = (value: string) => Number(value.replace(/[^0-9.-]/g, ''))
const parseAmountMinor = (value: string) => Math.round(parseAmountNumber(value) * 100)
...
const normalizePostedDate = (value: string) => {
...
const buildFallbackTransactionSourceRecordId = (
  sourceAccountId: string,
  postedAt: string,
  description: string,
  amountMinor: number,
  direction: string
) => `txn-${hashParts([sourceAccountId, postedAt, description, amountMinor, direction]).slice(0, 16)}`
  ...
  const transaction = db.transaction(() => {
    rows.forEach((row, index) => {
      insertRaw.run(
        batchId,
        index + 1,
        readField(row, ['source_record_id', 'id', 'Reference']) ?? null,
        readField(row, ['account_id', 'account', 'Account Number']) ?? null,
        readField(row, ['posted_at', 'date', 'Date']) ?? null,
        readField(row, ['description', 'Transaction Details']) ?? null,
        readField(row, ['amount', 'Amount']) ?? null,
        readField(row, ['balance', 'Balance']) ?? null,
        readField(row, ['category', 'Category']) ?? null,
        JSON.stringify(row)
      )
      ...
      try {
        ...
        const sourceRecordId =
          readField(row, ['source_record_id', 'id', 'Reference']) ??
        ...
        const accountId = ensureAccount(
          ...
          sourceName,
          sourceAccountId,
          readField(row, ['account_name', 'Account Name']) ?? sourceAccountId
        )
        ...
        const result = upsertCore.run(
          hashParts([sourceName, sourceRecordId]).slice(0, 24),
          accountId,
          sourceName,
          sourceRecordId,
          postedAt,
          settledAtRaw ? normalizePostedDate(settledAtRaw) : null,
          description,
          readField(row, ['merchant', 'Merchant Name']) ?? null,
          readField(row, ['category', 'Category']) ?? 'Uncategorised',
          readField(row, ['subcategory', 'Transaction Type']) ?? null,
          amountMinor,
