...
const parseAmountMinor = (value) => Math.round(Number(value.replace(/[^0-9.-]/g, '')) * 100);
  ...
  `);
    const transaction = db.transaction(() => {
        rows.forEach((row, index) => {
            insertRaw.run(batchId, index + 1, readField(row, ['source_record_id', 'id']) ?? null, readField(row, ['account_id', 'account']) ?? null, readField(row, ['posted_at', 'date']) ?? null, readField(row, ['description']) ?? null, readField(row, ['amount']) ?? null, readField(row, ['balance']) ?? null, readField(row, ['category']) ?? null, JSON.stringify(row));
            try {
                ...
                upsertCore.run(createHash('sha1').update(dedupeHash).digest('hex').slice(0, 24), accountId, sourceName, sourceRecordId, postedAt, readField(row, ['settled_at']) ?? null, description, readField(row, ['merchant']) ?? null, readField(row, ['category']) ?? 'Uncategorised', readField(row, ['subcategory']) ?? null, amountMinor, readField(row, ['currency']) ?? 'AUD', direction, balanceRaw ? parseAmountMinor(balanceRaw) : null, readField(row, ['note', 'notes']) ?? null, dedupeHash);
                insertedCount += 1;
            }
            catch (error) {
                rejectedCount += 1;
