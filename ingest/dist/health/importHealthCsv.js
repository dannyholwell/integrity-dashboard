    ...
    const transaction = db.transaction(() => {
        rows.forEach((row, index) => {
            ...
            try {
                ...
                upsertCore.run(day, sourceName, Number(readField(row, ['steps']) ?? 0), toNumberOrNull(readField(row, ['active_calories', 'calories'])), toNumberOrNull(readField(row, ['resting_heart_rate', 'resting_hr'])), toNumberOrNull(readField(row, ['hrv_ms', 'hrv'])), toNumberOrNull(readField(row, ['sleep_minutes'])), toNumberOrNull(readField(row, ['workout_minutes'])), toNumberOrNull(readField(row, ['body_weight_kg'])), toNumberOrNull(readField(row, ['recovery_score'])), toNumberOrNull(readField(row, ['oxygen_saturation_pct', 'oxygen'])));
                insertedCount += 1;
            }
            catch (error) {
                rejectedCount += 1;
